...
- フェーズ2：実装済み・**実音声での確認待ち**（`core/align.py` アライメントとセクション時刻、`core/plan.py` フレーム化と plan.json、`core/ffmpeg.py` ffprobe、`core/config.py`、単体テスト追加で計66件）。ユーザーの希望で音声なしで先に作成。開発環境からは Whisper モデルも音声合成も取得できず、stable-ts の実行は未確認
