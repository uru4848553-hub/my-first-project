...
class PlaceTest(unittest.TestCase):
    ...
    def test_duration_mismatch_is_error(self):
        ...
        def short(infos):
            placed = orig(infos)
            for p in placed:
                if p.item.path.endswith(".png") and "_freeze" in p.item.path:
                    p.duration = 5   # 静止画が伸びなかった
            return placed
        project.pool.AppendToTimeline = short
        with self.assertRaises(PlaceError) as cm:
            self.run_place(project)
        self.assertIn("S03[a]", str(cm.exception))
        self.assertIn("実際: 90から5フレーム", str(cm.exception))
